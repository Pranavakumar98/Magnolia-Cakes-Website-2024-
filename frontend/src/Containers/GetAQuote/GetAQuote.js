...
const GetAQuote = ({ api }) => {
  ...
  const validateFiles = (files) => {
    ...
    if (invalidFiles.length > 0) {
      setFileError("Invalid file type(s). Allowed types are: PNG, JPEG, JPG, TXT, DOCX, CSV, and PDF.");
      return false;
    }
    setFileError(""); // Clear the error when all files are valid
    return true;
  };
  ...
  const sendEmailHandler = async (e) => {
    ...
    contactFormData.append("message", body);
    if (files.length > 0) {
      files.forEach((f) => contactFormData.append("file", f));
    }
  ...
  const removeFile = (file) => {
    ...
    setFiles(newFiles);
    validateFiles(newFiles); // Revalidate after removing a file
  };
